...
(function(dice) {
    ...
    this.dice_box = function(container, dimentions) {
        ...
        var ambientLight = new THREE.AmbientLight($t.dice.ambient_light_color);
    ...
    this.dice_box.prototype.generate_vectors = function(notation, vector, boost) {
        ...
        notation.set.forEach(i => {
            ...
            var inertia = $t.dice.dice_inertia[i];
            var angle = {
    ...
    this.dice_box.prototype.check_if_throw_finished = function() {
        var res = true;
        var e = 6;
        if (this.iteration < 10 / $t.dice.frame_rate) {
    ...
    function shift_dice_faces(dice, value, res) {
        var r = $t.dice.dice_face_range[dice.dice_type];
        ...
        if (dice.dice_type == 'd4' && num != 0) {
            ...
            dice.material = new THREE.MeshFaceMaterial(
                $t.dice.create_d4_materials($t.dice.scale / 2, $t.dice.scale * 2, d4_labels[num]));
        }
        dice.geometry = geom;
    }
    ...
    this.dice_box.prototype.draw_selector = function() {
        ...
        for (var i = 0, pos = -3; i < $t.dice.known_types.length; ++i, ++pos) {
            var dice = $t.dice['create_' + $t.dice.known_types[i]]();
